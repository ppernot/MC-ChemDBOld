...
source_dir =paste0('../../../Reactor_Runs/ChemDBPublic/PhotoProcs_v_1.2/',reso,'nm/')
# if(test)
...
for (sp in species) {
  ...
  x = read.table(paste0(source_dir,prefix,'se',sp,'.dat.gz'),
                 col.names=1:10,
  ...
  for (j in 1:length(reacs)) {
    ...
    if (channel != 0) {
      iMC = 0
      prefix=paste0(sprintf('%04i',iMC),'_')
      x = read.table(paste0(source_dir,prefix,'qy',sp,'_',
                            channel,'.dat.gz'))
      wavq  = x[, 1]
      ...
      lines(wavq,qy[,1],col=ch_cols[channel],lty=1,lwd=8)
